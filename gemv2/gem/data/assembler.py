...
from ..core.data import DatasetSpec, GlobalDataset
from .utils import to_clean_list


class DatasetAssembler(ABC):
    """Abstract base for assembling source frames into a GlobalDataset."""

    @abstractmethod
    def assemble(self, source_dict: Dict[str, pl.DataFrame]) -> GlobalDataset:
        """Assemble source frames into a `GlobalDataset`."""


# Backwards-compatible alias
GlobalDataAssembler = DatasetAssembler


class FeatureAssembler(DatasetAssembler):
    """Assemble multiple source tables into one model-ready `GlobalDataset`."""
