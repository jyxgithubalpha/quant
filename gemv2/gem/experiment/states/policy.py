...
def _result_importance_vector(result: SplitResult) -> Optional[np.ndarray]:
    if result.state_delta is not None and result.state_delta.importance is not None:
        return result.state_delta.importance
    return result.importance_vector


def _result_feature_hash(result: SplitResult) -> str:
    if result.state_delta is not None and result.state_delta.feature_hash:
        return result.state_delta.feature_hash
