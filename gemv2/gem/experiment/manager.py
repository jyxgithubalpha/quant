...
from ..core.data import GlobalDataset, SplitSpec
...
class ExperimentManager:
    def __init__(
        ...
        self._global_dataset: Optional[GlobalDataset] = None
    ...
    def feature_names(self) -> Optional[List[str]]:
        if self._global_dataset is None:
            return None
        return self._global_dataset.feature_name_list
    ...
    def run(self) -> Dict[int, SplitResult]:
        ...
        log.info("[2/6] Building dataset...")
        self._global_dataset = self.data_module.build_dataset(
            gen_output.date_start,
            gen_output.date_end,
        )
        log.info("  - Samples: %d", self._global_dataset.n_samples)
        log.info("  - Features: %d", self._global_dataset.n_features)
