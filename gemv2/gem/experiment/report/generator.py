...
class ReportGenerator:
    ...
    def _aggregate_daily_metric_series(series_rows: List[Dict[str, object]]) -> pl.DataFrame:
        ...
        required = {"split", "metric", "date", "value"}
        ...
        selected_cols = [col for col in ("split_id", "split", "metric", "date", "value") if col in raw.columns]
        df = raw.select(selected_cols).with_columns(
            pl.col("split").cast(pl.Utf8, strict=False),
        ...
        drop_cols = ["split", "metric", "date", "value"] + (["split_id"] if has_split_id else [])
        ...
        return (
            ...
            .select(["date", "split", "metric", "value", "n_split", "is_derived", "source_metric"])
            .sort(["metric", "split", "date"])
        )
    ...
    def _append_derived_daily_metrics(daily_df: pl.DataFrame) -> pl.DataFrame:
        ...
        top_ret_df = base.filter(pl.col("metric") == "daily_top_ret").sort(["split", "date"])
        if not top_ret_df.is_empty() and "daily_top_ret_std" not in existing_metrics:
            top_ret_std_parts: List[pl.DataFrame] = []
            for split in top_ret_df["split"].unique().to_list():
                split_df = top_ret_df.filter(pl.col("split") == split).sort("date")
                if split_df.is_empty():
                    continue
                vals = split_df["value"].to_numpy()
                n = vals.shape[0]
                if n == 0:
                    continue
        ...
        if "daily_icir_expanding" not in existing_metrics:
            ic_df = base.filter(pl.col("metric") == "daily_ic").sort(["split", "date"])
            if not ic_df.is_empty():
                icir_parts: List[pl.DataFrame] = []
                for split in ic_df["split"].unique().to_list():
                    split_df = ic_df.filter(pl.col("split") == split).sort("date")
                    if split_df.is_empty():
                        continue
                    vals = split_df["value"].to_numpy()
                    n = vals.shape[0]
        ...
        return pl.concat(frames, how="vertical").sort(["metric", "split", "date"])
