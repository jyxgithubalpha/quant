...
from ..core.data import GlobalDataset
...
class LocalExecutor(BaseExecutor):
    ...
    def submit_run_split(
        ...
        return self.runner.run(task, global_dataset, state, ctx)
...
class RayExecutor(BaseExecutor):
    ...
    def _create_remote_funcs(self) -> None:
        ...
        @ray.remote
        def run_split_remote(task, global_dataset, state, ctx):
            from .split_runner import SplitRunner

            runner = SplitRunner()
            return runner.run(task, global_dataset, state, ctx)
    ...
    def submit_run_split(
        ...
        return remote_func.remote(task, global_dataset_ref, state_ref, ctx)
