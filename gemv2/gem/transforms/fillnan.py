"""
FillNaN -- fill missing values with constant / mean / median.
"""
...
class FillNaN(BaseTransform):
    """Fill NaN values using a specified strategy."""
    ...
    def fit(self, X, y, keys=None):
        if self.method == "constant":
            ...
            self._fill_values_y = (
                np.nanmean(y, axis=0) if y.ndim > 1 else np.array([np.nanmean(y)])
            )
        elif self.method == "median":
            self._fill_values_X = np.nanmedian(X, axis=0)
            self._fill_values_y = (
                np.nanmedian(y, axis=0) if y.ndim > 1 else np.array([np.nanmedian(y)])
            )
        return self

    def _fill_array(self, arr, fill_values, keys):
        out = arr.copy()
        if self.per_date and keys is not None and self.method in ("mean", "median"):
            for key in np.unique(keys):
                ...
                sub = out[mask]
                fill_vals = (
                    np.nanmean(sub, axis=0)
                    if self.method == "mean"
                    else np.nanmedian(sub, axis=0)
                )
                ...
                if nan_mask.any():
                    if sub.ndim > 1:
                        for i in range(sub.shape[1]):
                            col_nan = nan_mask[:, i]
                            fv = fill_vals[i] if not np.isnan(fill_vals[i]) else self.value
                            sub[col_nan, i] = fv
                    else:
                        fv = fill_vals if not np.isnan(fill_vals) else self.value
                        sub[nan_mask] = fv
                out[mask] = sub
        else:
            if out.ndim > 1:
