...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=4)  # batch中样本含几天
    ...
    def __init__(self, args):
        ...
        for layer_idx, (size_in, size_out) in enumerate(zip(layer_size_list[:-1], layer_size_list[1:])):
            if layer_idx <= 0:
                ...
                self.layers.add_module(f'Tanh{layer_idx + 1}', nn.Tanh())
...
def train(args, name, market, season, state='train'):
    ...
    params.all_data = pd.read_feather(rf'{fac_path}/{fac_name}/{sel_fac_name_save}')
